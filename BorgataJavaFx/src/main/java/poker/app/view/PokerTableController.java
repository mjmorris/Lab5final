	...
	private void handleDraw() {
		...
		if (act.isbEvaluateHand()) {
			...
			for(Card c: WinningHand.getCardsInHand()){
				...
				Bounds bndCardDealt = imgvCardDealt.localToScene(imgvCardDealt.getBoundsInLocal());
				Point2D pntCardDealt = new Point2D(bndCardDealt.getMinX(), bndCardDealt.getMinY());
				SequentialTransition win = new SequentialTransition();
				win.getChildren().add(createTransition(pntCardDealt, new Point2D(pntCardDealt.getX(), pntCardDealt.getY()+24), imgvCardDealt));
